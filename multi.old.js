...
async function main(input, desired) {
    ...
    for (let desired of [2, 8, 64, 256]) {        
        // For each of the selected nodes, calculate a weighted average to represent all its descendants
        const colours = new Map(colourMap) // Clone the colour map for each new selection set
        ...
        for (let node of selected) {
            ...
            if (node.key) colours.set(node.value.key(), newColour)
            ...
            for (let pair of node.pairsExcluding(selectedSet)) {
                colours.set(pair.value.key(), newColour)
        ...
        out.scan(0, 0, out.bitmap.width, out.bitmap.height, function(x, y, idx) {
            colours.get(PixelColour.keyFromBitmap(this.bitmap.data, idx)).setBitmap(this.bitmap.data, idx)
        })

        // Save the image and pallet under a new name
        out.write(noExtPath+`_reduced_${desired}.png`)
        saveColourPallet(colours.values(), `${noExtPath}_pallet_${desired}.png`)
    }
}

if (process.argv.length != 3) {
    console.log("Command requires 2 arguments: <inputPath>")
} else {
    main(process.argv[2])
}