...
async function main(input, desired) {
    ...
    console.log(`Read ${image.bitmap.width}x${image.bitmap.height} pixels containing ${colourMap.size} unique colours`)
    ...
    for (let desired of desiredList) {
        ...
        for (let index = 0; index < desired; index++) {
            ...
            node.value.replacement = newColour

            // For all descendants, set their replacement colour and add their frequencies
            for (let colour of node.valuesExcluding(selectedSet)) {
                sum.addInPlace(colour.vector3Scaled())
                colour.replacement = newColour
